...
var mainPanel = ui.Panel({
    ...
    function updateMap() {
      ...
      if (region) {
        ...
        if (imageType.indexOf('COPERNICUS/S2_SR_HARMONIZED') !== -1) {
          ...
          // Landsat-5和Landsat-7使用B3、B2、B1作为RGB真彩色波段（红、绿、蓝）
          displayBands = ['B3', 'B2', 'B1'];  // B3-红, B2-绿, B1-蓝
          visParams = {
            min: 0,
            max: 0.4,
            bands: displayBands
         ...
         */
        // 重新获取影像集合（未经median合成的原始集合）
        var imageCollectionForCoverage;
        if (imageType.indexOf('COPERNICUS/S2') !== -1) {
          imageCollectionForCoverage = imageCollection
            .filterDate(start, end)
            .filterBounds(region)
            .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 20));
        } else if (imageType.indexOf('LANDSAT') !== -1) {
        ...
        if (imageType.indexOf('COPERNICUS/S2') !== -1) {
          ...
          coverageBand = null; // 使用第一个可用波段
        }
        
        // 计算覆盖度
        var coverage;
        if (coverageBand) {
          coverage = imageCollectionForCoverage.select(coverageBand).count().clip(region);
        } else {
          coverage = imageCollectionForCoverage.count().clip(region);
        }
        
        // 可视化覆盖度图层（颜色：蓝色=1次，橙色=低覆盖，黄色=中等，绿色=高覆盖）
        map.addLayer(coverage, 
          {min: 1, max: 10, palette: ['blue', 'orange', 'yellow', 'lightgreen', 'green']}, 
          '影像覆盖度 (观测次数)');
        
        // 打印影像数量
        print('========== 覆盖度信息 ==========');
        print('影像数量:', imageCollectionForCoverage.size());
        print('说明: 地图上蓝色=1次观测，橙色=低覆盖，黄色=中等，绿色=高覆盖，无颜色=0次（无数据）');
        print('====================================');
    ...
      var scale;
      ...
      switch (selectedImage) {
        case 'COPERNICUS/S2_SR_HARMONIZED':
          // Sentinel-2 SR的B2-B12光谱波段（不包含辅助波段AOT、WVP、SCL、TCI等）
          bandNames = ['B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B8A', 'B9', 'B11', 'B12'];
          scale = 10;
          break;
        case 'COPERNICUS/S2_HARMONIZED':
          // Sentinel-2 TOA所有科学波段
          bandNames = ['B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B8A', 'B9', 'B10', 'B11', 'B12'];
          scale = 10;
          break;
        case 'CLOUD_SCORE_PLUS_HARMONIZED/S2_HARMONIZED_V1':
          // Cloud Score Plus波段
          ...
          break;
        case 'LANDSAT/LT05/C02/T1_TOA':
          // Landsat-5 TOA光谱波段（B2-B7，包括热红外B6）
          bandNames = ['B2', 'B3', 'B4', 'B5', 'B6', 'B7'];
          scale = 30;
          break;
        case 'LANDSAT/LE07/C02/T1_TOA':
          // Landsat-7 TOA光谱波段（B2-B7，包括热红外B6，不含全色B8）
          bandNames = ['B2', 'B3', 'B4', 'B5', 'B6', 'B7'];
          scale = 30;
          break;
        case 'LANDSAT/LC08/C02/T1_TOA':
          // Landsat-8 TOA所有波段（包括热红外B10和B11）
          bandNames = ['B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8', 'B9', 'B10', 'B11'];
          scale = 30;
          break;
        case 'LANDSAT/LC08/C02/T1':
          // Landsat-8 原始数据所有波段
      ...
      // ========== 在地图上显示影像和覆盖度 ==========
      // 1. 显示合成影像
      map.addLayer(data, visualization, selectedRegionName + ' 合成影像');
      ...
      map.addLayer(outline, {palette: 'red'}, selectedRegionName + ' 边界');
      ...
      print('说明：导出的GeoTIFF文件包含B2-B12光谱波段（共11个），不包含辅助波段');
      print('========================================================');
