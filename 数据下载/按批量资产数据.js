 ...
 * - 筛选指定时间范围内的哨兵-2影像
 * - 应用云量过滤和空间过滤
 * - 计算中值合成影像，只保留B2-B12光谱波段
 * - 不包含辅助波段（AOT, WVP, SCL, TCI等）
 * 出参:
 * - ee.Image: 合成后的哨兵-2影像，包含B2-B12光谱波段（共11个）
 */
...
function exportImageData(image, region, description) {
  ...
  print('说明：GeoTIFF包含B2-B12光谱波段（共11个），不包含辅助波段');
  print('数值说明：');
  print('- 原始值范围：0-10000（地表反射率×10000）');
  print('- 真实反射率：原始值÷10000，范围0-1');
  print('- 显示建议：RGB显示时设置最大值3000（对应反射率0.3）');
  print('====================================================');
}
